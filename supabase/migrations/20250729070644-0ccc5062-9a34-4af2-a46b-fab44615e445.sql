-- COMPREHENSIVE WAREHOUSE SIMULATION DATA MIGRATION - FINAL VERSION
-- Fixed window function issue and using correct constraints
...
-- Phase 5: Bin Assignments (fixed window function issue)
...
SELECT 
  gen_random_uuid(),
  ranked_bins.warehouse_bin_id,
  ranked_bins.inventory_item_id,
  CASE 
    WHEN ranked_bins.bin_type = 'bulk' THEN (ranked_bins.par_level * 2)
    WHEN ranked_bins.bin_type = 'standard' THEN ranked_bins.par_level
    ELSE (ranked_bins.par_level / 2)
  END as quantity,
  CASE WHEN ranked_bins.bin_type = 'pick' THEN 5 ELSE 0 END as reserved_quantity,
...
FROM (
  SELECT 
    ...
    ii.par_level,
    wb.bin_type,
    ROW_NUMBER() OVER (PARTITION BY ii.id ORDER BY wb.bin_code) as rn
  FROM warehouse_bins wb
  JOIN inventory_items ii ON ii.business_id = wb.business_id
) ranked_bins
WHERE ranked_bins.rn = 1;
...
-- Phase 7: Purchase Order Line Items with calculated line_total (fixed window function)
...
SELECT 
  gen_random_uuid(),
  ranked_po_items.purchase_order_id,
  ranked_po_items.inventory_item_id,
  ranked_po_items.barcode,
  ranked_po_items.name,
  ranked_po_items.ordered_quantity,
  ranked_po_items.received_quantity,
  ranked_po_items.current_cost,
  ranked_po_items.ordered_quantity * ranked_po_items.current_cost,
  'LOT-' || TO_CHAR(CURRENT_DATE, 'YYYYMMDD') || '-' || LPAD((RANDOM() * 1000)::integer::text, 3, '0'),
  CASE WHEN ranked_po_items.shelf_life_days IS NOT NULL THEN CURRENT_DATE + ranked_po_items.shelf_life_days ELSE NULL END
FROM (
  SELECT 
    ...
    ii.barcode,
    ii.name,
    (ii.minimum_order_quantity + (RANDOM() * ii.minimum_order_quantity)::integer) as ordered_quantity,
    CASE 
    ...
    ii.current_cost,
    ii.shelf_life_days,
    ROW_NUMBER() OVER (PARTITION BY po.id ORDER BY ii.name) as rn
  FROM purchase_orders po
  JOIN inventory_items ii ON ii.business_id = po.business_id
) ranked_po_items
WHERE ranked_po_items.rn <= 3;
